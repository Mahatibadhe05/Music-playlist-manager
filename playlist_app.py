...
def delete_song():
    if not user_playlist:
        print("🚫 You have no personal songs to delete.")
        return
    for idx ,song in enumerate(user_playlist,1):
        print(f"{idx}. {song['title']} by {song['artist']}")
    choice = int(input("Enter the number of song you want to delete:"))
    removed = user_playlist.pop(choice - 1)
    save_user_playlist(username, user_playlist)
